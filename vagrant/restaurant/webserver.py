...
class webServerHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            if self.path.endswith("/edit"):
                ...
                myRestaurantQuery = session.query(Restaurant).filter_by(id = idPath).one()
                if myRestaurantQuery:
                    ...
                    output += "<input name = 'restaurantName' type = 'text' placeholder = '%s' > " % myRestaurantQuery.name
                    output += "<input type='submit' value='Submit'>"
                    output += "</form></body></html>"
                    self.wfile.write(output)
                    return
    ...
    def do_POST(self):
        try:
            # Objective 4 - Make POST method for delete
            if self.path.endswith("/delete"):
                idPath = self.path.split('/')[2]
            ...
            # Objective 4 - Make POST method for edit
            if self.path.endswith("/edit"):
